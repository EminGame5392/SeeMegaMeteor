...
public class SQLiteDataStore implements DataStore {
    ...
    public SQLiteDataStore(File dbFile) throws SQLException {
        this.dbFile = dbFile;
    ...
    public void init(Consumer<Boolean> callback) {
        Bukkit.getScheduler().runTaskAsynchronously(SeeMegaMeteor.get(), () -> {
            try (Statement stmt = connection.createStatement()) {
                stmt.executeUpdate("CREATE TABLE IF NOT EXISTS loot (id INTEGER PRIMARY KEY AUTOINCREMENT, item BLOB, chance REAL)");
                verifyDataIntegrity();
                loadCache();
    ...
    public void convertTo(DataStore newStore, Consumer<Boolean> callback) {
        loadLoot(result -> {
            newStore.saveLoot(result, success -> {
                if (success) backup(backupSuccess -> {
                    if (callback != null) callback.accept(backupSuccess);
                });
                else if (callback != null) callback.accept(false);
            });
        });
    }
    ...
    public void backup(Consumer<Boolean> callback) {
        Bukkit.getScheduler().runTaskAsynchronously(SeeMegaMeteor.get(), () -> {
            ...
            String backupName = "loot_" + System.currentTimeMillis() + ".db";
    ...
    private void cleanupOldBackups(File backupDir) {
        File[] backups = backupDir.listFiles((dir, name) -> name.startsWith("loot_") && name.endsWith(".db"));
